...
export default {
  ...
  created() {
    const _this = this
    axios.get('http://localhost:8081/topic/NO?current=1&size=' + this.pageSize).then(function (resp) {
  ...
  methods: {
    page(currentPage) {
      const _this = this
      axios.get('http://localhost:8081/topic/NO?current=' + currentPage + '&size=' + this.pageSize).then(function (resp) {
